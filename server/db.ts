...
interface DatabaseData {
  ...
  pendingAccessRequests: { id: number; name: string; password: string; role: 'user' | 'admin' | 'monitor' | 'aprendiz' | 'corpo_docente'; function: string; requestedAt: string }[];
  editedProfessorNames: Record<number, string>;
  editedProfessorTipoContrato: Record<number, string>;
  userRoles: Record<number, 'user' | 'admin' | 'monitor' | 'aprendiz' | 'corpo_docente'>;
...
export async function createUser(data: Partial<User>) {
  ...
  const newUser: User & { approved: boolean } = {
    ...
    email: null,
    role: data.role === "admin" ? "admin" : (data.role === "monitor" ? "monitor" : (data.role === "aprendiz" ? "aprendiz" : (data.role === "corpo_docente" ? "corpo_docente" : "user"))),
    loginMethod: "password",
    approved: true,
    createdAt: new Date(),
    updatedAt: new Date(),
...
export async function upsertUser(data: Partial<User> & { openId: string }): Promise<User> {
  ...
  if (existing) {
    const role = (existing.id === 1 || data.role === "admin") ? "admin" : (data.role === "monitor" ? "monitor" : (data.role === "aprendiz" ? "aprendiz" : (data.role === "corpo_docente" ? "corpo_docente" : "user")));
  ...
  const newUser: User & { approved: boolean } = {
    ...
    role: (data.role === "admin" ? "admin" : (data.role === "monitor" ? "monitor" : (data.role === "aprendiz" ? "aprendiz" : (data.role === "corpo_docente" ? "corpo_docente" : "user")))) as any,
    approved: true,
    createdAt: now,
    updatedAt: now,
    lastSignedIn: now
...
export async function requestAccess(name: string, password: string, role: 'user' | 'admin' | 'monitor' | 'aprendiz' | 'corpo_docente', userFunction: string) {
  await db.read();
  const hashedPassword = await hashPassword(password);
  const request = {
...
export async function approveAccess(requestId: number, updates?: { name?: string; password?: string; role?: 'user' | 'admin' | 'monitor' | 'aprendiz' | 'corpo_docente'; function?: string }) {
  await db.read();
