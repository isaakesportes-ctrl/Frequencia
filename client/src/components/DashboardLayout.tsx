...
function DashboardLayoutContent({
  ...
  // Simulação de role para o usuário
  const isAdmin = user?.email?.includes("admin") || user?.displayName?.includes("Admin");
  const isProfessor = user?.email?.includes("prof") || user?.displayName?.includes("Professor");
  const isFrequencia = user?.email?.includes("frequencia") || user?.displayName?.includes("Frequência");
  ...
  if (isFrequencia) {
    ...
    // Se for apenas frequência, mas não tem indicação, pode ser professor ou admin?
    // Vamos por padrão exibir o menu de professor, mas se quiser, pode mudar
    menuItems = PROFESSOR_MENU;
  }
